    ...
    for line in lines:
        if seperator in line and len(line) > len(seperator):
            try:
                ...
                head_vocab.add(line.split(',')[2].strip())
            except:
                pass
...
def read_data(path):
    # fixme: save all path to file paper.pt
    if path is None:
        return None
...
def load_wiki727k(args):
    train_path, valid_path, test_path = args.train, args.valid, args.test

    papers_train_path, papers_valid_path, papers_test_path = read_data(train_path), read_data(valid_path), read_data(
        test_path)

    word_vocab, topic_vocab, head_vocab = build_vocab(papers_train_path, train_path, args.rebuild_vocab)

    mean_windows_size = cal_mean_windows_size(train_path, args.rebuild_vocab, papers_train_path, papers_valid_path,
                                              papers_test_path)
