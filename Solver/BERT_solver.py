...
class BERT_Solver(Solver):
    ...
    def read_file(self, path, reconstruct=True):
        paper = Paper(path, reconstruct, self.topic_vocab.word2int, self.sentence_encoder_dict, self.device)
        return paper
    ...
    def collate_fn(self, batch):
        ...
        for path in batch:
            paper = self.read_file(path, reconstruct=False)
            ...
            sentence_embedding_papers.append(torch.from_numpy(paper.bert_embedding).to(self.device))
        ...
        if mode == 'train':
            ...
            for sentence_embedding_papers, topic_label_papers, segment_label_papers in tqdm(data_loader):
                ...
                if not self.model_need_enforced_teach_and_segment_label:
                    ...
                    segmentation_probability, segmentation_identification, topic_classification_probability = self.model(
                        sentence_embedding_papers, tensor_segment_label_papers, True if epoch < self.epoch else False)
            ...
            for sentence_embedding_papers, topic_label_papers, segment_label_papers in tqdm(data_loader):
                ...
                if not self.model_need_enforced_teach_and_segment_label:
                    ...
                    segmentation_probability, segmentation_identification, topic_classification_probability = self.model(
                        sentence_embedding_papers, tensor_segment_label_papers, False)
