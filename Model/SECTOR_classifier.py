...
class Sector_Classifier(nn.Module):
    def __init__(self, topic_size, GRU=False, input_dim=768, hidden_dim=128, output_dim=2, n_layers=2,
        ...
        self.Ws_topic_classification1 = nn.Parameter(torch.Tensor(hidden_dim * 2, int(math.sqrt(topic_size * hidden_dim))))
        self.bs_topic_classification1 = nn.Parameter(torch.zeros((int(math.sqrt(topic_size * hidden_dim)),)))

        self.Ws_topic_classification2 = nn.Parameter(torch.Tensor(int(math.sqrt(topic_size * hidden_dim)), topic_size))
        self.bs_topic_classification2 = nn.Parameter(torch.zeros((topic_size, )))

        self.activation = nn.ReLU()

        nn.init.uniform_(self.Ws_segmentation, -0.1, 0.1)
        nn.init.uniform_(self.Ws_topic_classification1, -0.1, 0.1)
        nn.init.uniform_(self.Ws_topic_classification2, -0.1, 0.1)

    @staticmethod
    def pad(sentences_embedding_papers):
    ...
    def forward(self, sentences_embedding_papers):
        ...
        #     self.activation(padded_sentences_embedding_after_rnn).matmul(self.Ws_topic_classification) + self.bs_topic_classification

        padded_topic_probability = \
            self.activation(self.activation(padded_sentences_embedding_after_rnn.matmul(
                self.Ws_topic_classification1)
                + self.bs_topic_classification1).matmul(self.Ws_topic_classification2) + self.bs_topic_classification2)
