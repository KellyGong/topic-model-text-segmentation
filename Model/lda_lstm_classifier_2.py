...
class LDA_LSTM_Classifier_2(nn.Module):
    def __init__(self, args, topic_size, lda_word_size, GRU=False, input_dim=768, hidden_dim=128, output_dim=2, n_layers=2,
                 bidirectional=True,
                 dropout=0.5):
        super(LDA_LSTM_Classifier_2, self).__init__()
        if GRU:
            self.rnn = nn.GRU(input_dim + args.num_topic, hidden_dim, num_layers=n_layers, bidirectional=bidirectional,
                              dropout=dropout, batch_first=True)
        else:
            self.rnn = nn.LSTM(input_dim + args.num_topic, hidden_dim, num_layers=n_layers, bidirectional=bidirectional,
                               dropout=dropout, batch_first=True)
        ...
        self.layer_norm = nn.LayerNorm(input_dim+args.num_topic)
    ...
    def forward(self, sentences_embedding_papers, lda_one_hot_papers, seg_label, enforced_teach=False):
        ...
        padded_sentences_embedding_concat_lda = \
            self.layer_norm(torch.cat((padded_sentences_embedding_papers, padded_lda_topic_vectors_papers.detach()), dim=-1))

        padded_sentences_embedding_after_rnn, (_) = self.rnn(padded_sentences_embedding_concat_lda)
